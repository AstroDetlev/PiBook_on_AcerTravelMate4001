...
/*
 each key gets its unique number here. There are 89 keys on the keyboard
 The strange order is the order as used in the matrix. othe rorders will work, too.
 But if the last button of this array can be used, at least the structures of this sketch are ok.
*/

#define ID_KEY_UNKNOWN 
#define ID_KEY_F11 1
#define ID_KEY_F12 2
#define ID_KEY_F8  3
#define ID_KEY_Q 4
    ...
    for(in=out+1; in<=(HIGHEST_PIN) ; in++)
    {
      ...
      if (out != in)
      {
        ...
        if (newstate == LOW)
        {
          ...
          int out2 = out;
          //remove the offset
          in2 = in2 - LOWEST_PIN +1;
          out2 = out2 - LOWEST_PIN +1; 
  
          out = int(Matrix[out2-1][in2-1]); //shift by 1 byte. The first entry is at [0][0]!
          ...
          if (out > 0)
          {
            ...
            //Serial.print(in2);
            //Serial.print(",");
            Serial.println(HumanReadableDecoder[out]);
            delay(200);
          }
